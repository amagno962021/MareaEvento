sap.ui.define([
    ...
    function (Controller, Utils, TasaBackendService, MessageBox, BusyIndicator) {
        ...
        return Controller.extend("com.tasa.mareaevento.controller.MainComp", {

            obtenerRouter: function () {
                return this.getOwnerComponent().getRouter();
            },

            navegar: function (nameRoute, params) {
                var oRouter = this.obtenerRouter();
                if (params) {
                    oRouter.navTo(nameRoute, params);
                } else {
                    oRouter.navTo(nameRoute);
                }
            },

            getModelo: function () {
                return this.getOwnerComponent().getModel("DetalleMarea");
            },

            backToExternalApp: function () {
                var oStore = jQuery.sap.storage(jQuery.sap.storage.Type.Session);
                var previousApp = oStore.get("AppOrigin");
                var oCrossAppNav = sap.ushell.Container.getService("CrossApplicationNavigation");
                oCrossAppNav.toExternal({
                    target: {
                        semanticObject: previousApp,
                        action: "display"
                    }
                });
            },
